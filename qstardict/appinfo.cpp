...
const QString AppInfo::genericDataDir() {
  ...
  typedef HRESULT(WINAPI * SHGetFolderPathWFunc)(HWND, int, HANDLE, DWORD,
						 LPTSTR);
  SHGetFolderPathWFunc SHGetFolderPathW =
      (SHGetFolderPathWFunc)QLibrary::resolve(QLatin1String("Shell32"),
					      "SHGetFolderPathW");
...
QStringList AppInfo::appPluginsDirs() {
  ...
  if (!(pluginsDir.cd("../plugins") || pluginsDir.cd("plugins"))) {
    qDebug() << "Can't find plugins dir directory, probably won't find plugins "
		"as well";
  }
